...
class TestWebhookManager:
    ...
    def test_register_with_filters(self, webhook_db):
        ...
        assert hook["events"] == ["validation.failed"]
        assert hook["contracts"] == ["customer"]

        stored = mgr.list_hooks()[0]
        assert stored["events"] == ["validation.failed"]
...
class TestWebhookAPI:
    ...
    def test_register_webhook_endpoint(self, client, auth_headers):
        r = client.post(
            "/api/v1/webhooks",
    ...
    def test_list_webhooks_endpoint(self, client, auth_headers):
    ...
    def test_unregister_webhook_endpoint(self, client, auth_headers):
    ...
    def test_register_and_verify_state(self, client, auth_headers):
        ...
        assert matched[0]["events"] == ["validation.failed"]
...
class TestWebhookSchemas:
    ...
    def test_new_fields_not_in_valid_events(self):
        ...
        assert VALID_EVENTS == {"validation.failed", "validation.warning", "batch.failed"}
