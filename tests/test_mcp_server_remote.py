...
@pytest.mark.asyncio
async def test_remote_list_contracts():
    api_payload = [
...
@pytest.mark.asyncio
async def test_remote_get_contract_latest():
    api_payload = {
    ...
    client = _mock_client(get=_mock_response(api_payload))
    with patch.object(mcp_server, "_remote_client", client):
        result = await _tool_get_contract({"name": "customer"})
    data = json.loads(result[0].text)
...
@pytest.mark.asyncio
async def test_remote_get_contract_specific_version():
    api_payload = {"name": "customer", "version": "2.0", "status": "active", "rules": []}
    client = _mock_client(get=_mock_response(api_payload))
    with patch.object(mcp_server, "_remote_client", client):
        await _tool_get_contract({"name": "customer", "version": "2.0"})
...
@pytest.mark.asyncio
async def test_remote_validate_record_pass():
...
@pytest.mark.asyncio
async def test_remote_validate_record_fail():
    api_payload = {
        "valid": False,
...
@pytest.mark.asyncio
async def test_remote_validate_record_with_context():
    api_payload = {"valid": True, "errors": [], "contract": "customer"}
    client = _mock_client(post=_mock_response(api_payload))
    with patch.object(mcp_server, "_remote_client", client):
        await _tool_validate_record({
            "contract": "customer",
...
@pytest.mark.asyncio
async def test_remote_validate_batch():
    api_payload = {
    ...
    with patch.object(mcp_server, "_remote_client", client):
        result = await _tool_validate_batch({"contract": "customer", "records": records})
    data = json.loads(result[0].text)
...
@pytest.mark.asyncio
async def test_remote_explain_error_not_empty():
    ...
    with patch.object(mcp_server, "_remote_client", client):
        result = await _tool_explain_error({
...
@pytest.mark.asyncio
async def test_remote_explain_error_rule_not_found():
...
@pytest.mark.asyncio
async def test_remote_explain_error_contract_not_found():
...
@pytest.mark.asyncio
async def test_remote_create_contract_draft_with_reload():
    ...
    with patch.object(mcp_server, "_remote_client", client):
        result = await _tool_create_contract_draft(args)
...
@pytest.mark.asyncio
async def test_remote_create_contract_draft_reload_failure_graceful():
