...
class TestImportODCSEndpoint:
    ...
    def test_returns_200_with_auth(self, client, auth_headers):
        r = client.post("/api/v1/import/odcs", json=MINIMAL_CONTRACT, headers=auth_headers)
        assert r.status_code == 200

    def test_response_has_contract(self, client, auth_headers):
        r = client.post("/api/v1/import/odcs", json=MINIMAL_CONTRACT, headers=auth_headers)
        data = r.json()
        assert "contract" in data

    def test_response_has_rule_count(self, client, auth_headers):
        r = client.post("/api/v1/import/odcs", json=MINIMAL_CONTRACT, headers=auth_headers)
        data = r.json()
        assert "rule_count" in data
        assert data["rule_count"] >= 2  # email: not_empty + unique; name: not_empty

    def test_response_has_skipped_checks(self, client, auth_headers):
        r = client.post("/api/v1/import/odcs", json=MINIMAL_CONTRACT, headers=auth_headers)
        assert "skipped_checks" in r.json()

    def test_rules_have_correct_fields(self, client, auth_headers):
        r = client.post("/api/v1/import/odcs", json=MINIMAL_CONTRACT, headers=auth_headers)
