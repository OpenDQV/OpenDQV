"""Tests for contract lifecycle (DRAFT -> ACTIVE -> DEPRECATED)."""
...
class TestDeprecatedFilter:
    """Test that deprecated contracts are filtered from default listing."""

    def test_deprecated_hidden_in_list(self):
        """Test via registry directly to avoid rate limit issues."""
        from api.routes import registry

        # Save original, set to deprecated
        contract = registry.get("customer")
        original_status = contract.status
        registry.set_status("customer", "latest", ContractStatus.DEPRECATED)

        # Default list hides deprecated
...
class TestActiveContractImmutability:
    ...
    def test_add_rule_blocked_on_active_writer(self, client, auth_headers):
        """Writer cannot add a rule to an ACTIVE contract — must get 409."""
    ...
    def test_add_rule_blocked_on_active_approver(self, client, approver_headers):
        """Approver cannot add a rule to an ACTIVE contract — 409 regardless of role."""
        name = self._get_active_contract(client, approver_headers)
        if not name:
            pytest.skip("No active contracts available")
        r = client.post(f"/api/v1/contracts/{name}/rules", json=self._RULE, headers=approver_headers)
        assert r.status_code == 409
        assert "ACTIVE" in r.json()["detail"]

    def test_update_rule_blocked_on_active(self, client, approver_headers):
        """update_rule returns 409 on ACTIVE contracts regardless of role."""
        name = self._get_active_contract(client, approver_headers)
        if not name:
            pytest.skip("No active contracts available")
        r = client.get(f"/api/v1/contracts/{name}", headers=approver_headers)
        ...
        r = client.put(f"/api/v1/contracts/{name}/rules/{rule['name']}", json=payload, headers=approver_headers)
        assert r.status_code == 409
        assert "ACTIVE" in r.json()["detail"]

    def test_delete_rule_blocked_on_active(self, client, approver_headers):
        """delete_rule returns 409 on ACTIVE contracts regardless of role."""
        name = self._get_active_contract(client, approver_headers)
        if not name:
            pytest.skip("No active contracts available")
        r = client.get(f"/api/v1/contracts/{name}", headers=approver_headers)
        ...
        r = client.delete(f"/api/v1/contracts/{name}/rules/{rule_name}", headers=approver_headers)
        assert r.status_code == 409
        assert "ACTIVE" in r.json()["detail"]

    def test_409_detail_includes_remediation_path(self, client, approver_headers):
...
class TestRuleMutationOnDraft:
    ...
    def test_writer_can_add_rule_to_draft_contract(self, client, auth_headers, approver_headers):
        """Any authenticated user may add rules to a DRAFT contract."""
        ...
        try:
            r = client.post(f"/api/v1/contracts/{contract_name}/rules", json=self._RULE, headers=auth_headers)
            assert r.status_code == 200
            assert r.json()["status"] == "added"
            # Draft patch counter must appear in the version
            assert "-draft." in r.json()["version"]
            # Clean up rule
            client.delete(f"/api/v1/contracts/{contract_name}/rules/{self._RULE['name']}", headers=auth_headers)
        finally:
            client.post(f"/api/v1/contracts/{contract_name}/status", params={"status": "active"}, headers=approver_headers)

    def test_draft_patch_counter_increments_on_successive_mutations(self, client, auth_headers, approver_headers):
        ...
        try:
            ...
            r1 = client.post(f"/api/v1/contracts/{contract_name}/rules", json=rule_a, headers=auth_headers)
            assert r1.status_code == 200
            version_after_first = r1.json()["version"]
            assert "-draft." in version_after_first

            r2 = client.post(f"/api/v1/contracts/{contract_name}/rules", json=rule_b, headers=auth_headers)
            ...
            # Clean up
            client.delete(f"/api/v1/contracts/{contract_name}/rules/act047_counter_rule_a", headers=auth_headers)
            client.delete(f"/api/v1/contracts/{contract_name}/rules/act047_counter_rule_b", headers=auth_headers)
        finally:
            client.post(f"/api/v1/contracts/{contract_name}/status", params={"status": "active"}, headers=approver_headers)

    def test_delete_rule_on_draft_increments_counter(self, client, auth_headers, approver_headers):
        ...
        try:
            ...
            r = client.delete(f"/api/v1/contracts/{contract_name}/rules/{self._RULE['name']}", headers=auth_headers)
            assert r.status_code == 200
...
class TestXAuthModeHeader:
    ...
    def test_add_rule_has_x_auth_mode_header(self, client, auth_headers, approver_headers):
        ...
        r = client.post(f"/api/v1/contracts/{self._NAME}/rules", json=new_rule, headers=auth_headers)
        assert r.status_code == 200
        assert "x-auth-mode" in r.headers
...
class TestWriteGuardrailBypass:
    ...
    def test_add_rule_with_dry_run_false_still_blocked(self, client, auth_headers):
    ...
    def test_add_rule_with_dry_run_true_still_blocked(self, client, auth_headers):
    ...
    def test_update_rule_with_extra_params_still_blocked(self, client, auth_headers):
        """Extra query parameters on update_rule must not bypass the ACTIVE guardrail."""
        name = self._get_active_contract(client, auth_headers)
        if not name:
            pytest.skip("No active contracts available")
        r = client.get(f"/api/v1/contracts/{name}", headers=auth_headers)
        rules = r.json().get("rules", [])
        if not rules:
    ...
    def test_delete_rule_with_extra_params_still_blocked(self, client, auth_headers):
