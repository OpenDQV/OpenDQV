...
> **Get started in 15 minutes → [docs/quickstart.md](docs/quickstart.md)**

> **Building with LLMs or AI agents? Read [`llms.txt`](llms.txt) first — it covers the MCP server, REST API, and all six agent tools in two minutes.**
...
```
  ...
  Oracle --------+      |  (stateless)     |      + per-field errors
...
- **Governance built in.** Contract lifecycle (draft/active/deprecated), ownership, versioning, audit metrics.
...
# Development (live source reload, AUTH_MODE=open, UI on localhost only):
...
curl -s -X POST http://localhost:8000/api/v1/tokens/generate \
  ...
  -d '{"name": "my-first-token", "role": "admin"}'
```

Response:
```json
{"token": "opendqv_abc123...", "name": "my-first-token", "role": "admin"}
```
...
### Step 5: Airflow integration

> **API last verified:** Apache Airflow 2.x — 2026-03-13. [Check for updates](https://pypi.org/project/apache-airflow/)

Use `validate_batch` inside a `PythonOperator` to gate pipeline stages on data quality:
...
def validate_orders(**context):
    ...
    client = OpenDQVClient("http://opendqv.internal:8000", token="{{ var.value.opendqv_token }}")
    result = client.validate_batch(records, contract="order")
    summary = result["summary"]
    failure_rate = summary["failed"] / summary["total"] if summary["total"] else 0
    if failure_rate > 0.05:
        raise AirflowException(
            f"Order data quality check failed: {summary['failed']}/{summary['total']} records invalid "
            f"({failure_rate:.1%} > 5% threshold). Top rule: {list(summary['rule_failure_counts'].keys())[0]}"
        )

validate_task = PythonOperator(task_id="validate_orders", python_callable=validate_orders)
```

---

## Walkthrough: Adding OpenDQV to a Salesforce Integration

This is a real-world example of how you'd use OpenDQV to validate Salesforce Contact records before they're written.

### Step 1: Write a data contract

Create `contracts/sf_contact.yaml`:
...
### Step 3: Validate from your integration
...
### Step 4: Add to Salesforce via Apex trigger
...
### Step 5: Or generate embedded Apex code (push-down mode)

If your Salesforce org can't make HTTP callouts, generate the validation logic directly:

```bash
curl -X POST "http://localhost:8000/api/v1/generate?contract_name=sf_contact&target=salesforce&context=salesforce_prod"
```

This returns a complete Apex class with all the rules baked in.

---

## Data Contracts

**Data contracts** are versioned YAML files in the `contracts/` directory. Each contract defines the validation criteria for a business entity.
...
| `deprecated` | Still works but hidden from default listings. Callers should migrate. |
...
Every lifecycle transition is recorded in an append-only, hash-chained `ContractHistory` audit log (including `approved_by` identity), satisfying FCA SYSC, Ofwat, NHS DSP Toolkit, and SOX-adjacent data governance requirements.
...
| `GET` | `/api/v1/stats` | No | Validation statistics (for monitoring dashboard) |
| `POST` | `/api/v1/tokens/generate` | No | Generate a PAT |
| `POST` | `/api/v1/tokens/revoke` | No | Revoke a PAT |
...
Targets: `salesforce` (Apex class), `js` (JavaScript function), `snowflake` (JS UDF)
...
| `audit-verify` | Verify contract_history hash-chain integrity (`--db`) |
...
Benchmarked on a Dell XPS 13 (single Docker container, 1 Uvicorn worker), all security features active (ReDoS protection on, rate limiting disabled at app layer as recommended for reverse-proxy deployments):

| Run | Throughput | p50 | p99 | Total requests | Errors |
|-----|-----------|-----|-----|----------------|--------|
| 1 min | **194.9 req/s** | 4.16 ms | 13.99 ms | 11,692 | 0 |
| 5 min | **203.0 req/s** | 3.87 ms | 13.34 ms | 60,887 | 0 |
| 10 min | **199.2 req/s** | 4.00 ms | 13.46 ms | 119,545 | 0 |

Sustained throughput ~199 req/s. Zero errors across all runs. No degradation over time.
...
# Full pre-release smoke test (42 checks — unit, HTTP, pip install)
...
- [docs/snowflake_integration.md](docs/snowflake_integration.md) — Python connector; Snowpipe; External Function; Streams & Tasks
...
- [docs/databricks_integration.md](docs/databricks_integration.md) — Delta Lake; DLT quarantine; Jobs/Asset Bundles gate; Unity Catalog; Asset Bundles
...
- [docs/roadmap.md](docs/roadmap.md) — Planned integrations and features by quarter
...
- [docs/benchmark_throughput.md](docs/benchmark_throughput.md) — ~199 req/s benchmark results
