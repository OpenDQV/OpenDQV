...
async def validate_single(
    ...
    Only ACTIVE (and DEPRECATED) contracts can be validated against by default.
        ...
        if not contract:
            raise HTTPException(
                ...
                detail=f"Contract '{body.contract}' version '{body.version}' not found. "
                       f"Available: {[c['name'] for c in registry.list_contracts()]}",
            )
...
async def verify_trace_log_endpoint(
    ...
    """Verify the tamper-evident hash chain of the TRACE_LOG file."""
...
async def validate_batch_file(
    ...
    # SEC-012: configurable upload size limit (OPENDQV_MAX_UPLOAD_MB, default 10MB)
...
async def list_contracts(
    request: Request,
    include_all: bool = Query(False, description="Include DEPRECATED contracts"),
):
...
async def change_contract_status(
    ...
    response: Response,
    name: str,
    status: str = Query(..., description="New status: draft, active, or deprecated"),
    version: str = Query("latest"),
    user: str = Depends(get_current_user),
    ...
    Governance workflow: draft → active → deprecated

    Maker-checker: promoting a contract to 'active' requires the 'approver' or
    'admin' role. Writers may only set contracts to 'draft' or 'deprecated'.
    ...
    try:
        ...
        raise HTTPException(
            status_code=400,
            detail=f"Invalid status '{status}'. Must be: draft, active, deprecated",
        )
...
async def submit_for_review(
    ...
    if role not in ("editor", "approver", "admin"):
        raise HTTPException(
            status_code=403,
            detail=f"Role '{role}' is not permitted for this action. Required: ['editor', 'approver', 'admin']",
        )
...
@router.get("/contracts/{name}/history")
@_default_limit
...
@router.get("/contracts/{name}/diff")
@_default_limit
...
async def generate_token(
    ...
    role: str = Query("writer", description="Token role: writer, editor, approver, auditor, admin (default: writer)"),
    ...
    In AUTH_MODE=open, elevated roles (admin, approver) are capped to 'writer'.
    Admin and approver tokens can only be issued in AUTH_MODE=token to prevent
    privilege escalation tokens from being persisted in development environments.
    """
    ...
    # "token". Cap to "writer" in open mode regardless of what was requested.
    effective_role = role
    if config.AUTH_MODE == "open" and role in ("admin", "approver"):
...
@_tokens_limit
async def revoke_token(request: Request, token: str = Body(..., media_type="text/plain")):
    """Revoke a specific PAT by token value."""
    return revoke_pat(token)


@router.post("/tokens/revoke/{username}")
@_tokens_limit
async def revoke_system_tokens(request: Request, username: str, user=Depends(get_current_user)):
    """Revoke all tokens for a source system (e.g. when rotating credentials)."""
    ...
    if peer:
        try:
            async with httpx.AsyncClient(timeout=5.0) as http:
                ...
                if diverged:
                    webhook_manager.fire("sync_diverged", {
                        "opendqv_node_id": config.OPENDQV_NODE_ID,
                        "peer": peer,
