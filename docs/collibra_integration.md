...
## Approach 3 — Webhook → Collibra Workflow Trigger on `validation.failed`
...
Set `COLLIBRA_WORKFLOW_ID` to the UUID of your Collibra workflow definition. The workflow is triggered per `validation.failed` event — configure Collibra deduplication if the same asset fires frequently.
...
| **Q3 2026** | Schedule daily quality score push from OpenDQV trend → Collibra DQ Dimension |
| **Q3 2026** | Add webhook → Collibra workflow trigger for `validation.failed` events |
| **Q4 2026** | Rule-level Data Rule sync for full stewardship visibility |
