...
When OpenDQV fires `validation.failed`, route the event to an Azure Event Hub. Purview can subscribe to Event Hub topics for custom event ingestion.
...
| **Q2 2026** | Schedule daily quality score push from OpenDQV trend → Purview health metrics |
| **Q3 2026** | Add webhook → Event Hub pipeline for `validation.failed` alerting |
