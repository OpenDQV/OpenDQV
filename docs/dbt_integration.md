...
result = client.validate_batch(records, contract="orders", trace_id=dbt_run_id)
...
| **Q2 2026** | Add `trace_id` to dbt model runs; pass to OpenDQV for end-to-end lineage |
| **Q3 2026** | Publish `opendqv_dbt` macro package for singular tests |
| **Q4 2026** | Native dbt Cloud integration via webhook trigger on dbt job failure |
