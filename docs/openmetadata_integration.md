...
## Approach 3 — Webhook-Driven `opendqv.validation.failed` → Real-Time Test Case Result
...
OpenDQV engine
     │
     │  POST /webhooks/openmetadata (opendqv.validation.failed event)
     ▼
...
| **Planned — based on community demand** | Add Approach 5 (read-back incremental sync) to reduce API call volume |
| **Planned — based on community demand** | Deploy Approach 3 (webhook receiver) for real-time failure test case results |
| **Planned — based on community demand** | Add Approach 2 (native Test Suite + Test Case results) for DQ dashboard visibility |
| **Planned — based on community demand** | Add Approach 4 (rule-level test cases) for teams that monitor at rule granularity |
