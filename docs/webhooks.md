...
curl -X POST "http://localhost:8000/api/v1/webhooks" \
  ...
  -d '{
    ...
    "events": ["validation.failed", "batch.failed"],
...
|-------|----------------|
| `validation.failed` | A single-record validation returns `valid: false` (at least one error-severity rule failed) |
| `validation.warning` | A single-record validation returns `valid: true` but with non-empty `warnings` |
| `batch.failed` | A batch validation has at least one record with `valid: false` |
...
**`validation.failed` / `validation.warning`:**
...
async def receive_hook(request: Request):
    ...
    if body.get("event") == "validation.failed":
