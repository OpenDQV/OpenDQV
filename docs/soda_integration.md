...
## Approach 4 — Webhook-Driven: `validation.failed` → Soda Cloud Notification
...
| `export-soda` CLI command | Export from OpenDQV → Soda `checks.yml` is planned but not yet implemented (import is implemented) |
...
| **Q2 2026** | Add incremental hash-based sync into your CI/CD pipeline |
| **Q3 2026** | Webhook-triggered Soda Cloud notifications on validation failure cluster |
| **Q4 2026** | `export-soda` CLI command — generate Soda `checks.yml` from OpenDQV contracts |
