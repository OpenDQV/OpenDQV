...
> **You need:** Docker Desktop ([download here](https://www.docker.com/products/docker-desktop/)) or Python 3.9+. That's it.
...
**If you don't have Docker (Python 3.9+ only):** use the bootstrap script. The first run takes 2–3 minutes to install dependencies.
...
- **Token roles** — use `POST /api/v1/tokens/generate?username=alice&role=auditor` to create read-only tokens for external auditors. Roles: `writer` (default), `editor`, `approver`, `auditor`, `admin`. See [`docs/production_deployment.md`](production_deployment.md) for role permissions.
