...
| `ARCHIVED` | Still functional but hidden from default contract listings. Callers should migrate to a newer version. |

> **Note:** Rejection transitions the contract back to `DRAFT` (not to a separate `REJECTED` state). The rejection reason and `rejected_by` are recorded in the `ContractHistory` audit log for traceability.
...
```
POST /api/v1/contracts/{name}/status?status=archived
```

**Roles:** approver or admin
**Effect:** Marks the contract as archived. It continues to function but is hidden from default contract listings. Existing integrations will receive an archival notice in the response headers.

## Roles

| Role | Can submit | Can approve | Can reject | Can archive | Notes |
|------|-----------|------------|-----------|--------------|-------|
| validator | No | No | No | No | Default role for source-system tokens |
| reader | No | No | No | No | Read-only contract browsing |
| auditor | No | No | No | No | Compliance review and audit trail access |
| editor | Yes | No | No | No | Can author DRAFT contracts and submit for review |
| approver | No | Yes | Yes | Yes | Pure reviewer — cannot author contracts |
| admin | Yes | Yes | Yes | Yes | Full access including token management |

The separation between `editor` and `approver` roles enforces the maker-checker principle: the person who authored a contract cannot be the person who approves it for production. Notably, `approver` cannot submit contracts for review — only `editor` and `admin` can do that.
