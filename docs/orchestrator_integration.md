...
def orders_ingestion():
    ...
    def validate_with_opendqv(records: list[dict], **context) -> list[dict]:
        ...
        result = client.validate_batch(
            records,
            contract="orders",
            trace_id=run_id,  # correlate with Airflow run ID
        )
...
| **Q2 2026** | Switch to `AsyncOpenDQVClient` for high-throughput pipelines |
| **Q3 2026** | Add webhook back-channel to push OpenDQV rejections to your alerting stack |
