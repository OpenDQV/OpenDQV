...
```bash
# Generate a token for a writer
curl -X POST "http://localhost:8000/api/v1/tokens/generate?username=alice"

# Generate a token for an approver
curl -X POST "http://localhost:8000/api/v1/tokens/generate?username=bob&role=approver"

# Generate a token for an admin
curl -X POST "http://localhost:8000/api/v1/tokens/generate?username=admin&role=admin"
```
...
OpenDQV enforces a maker-checker model with five roles:

| Role | Permissions |
|------|------------|
| `writer` (default) | Create contracts in DRAFT; add/update/delete rules on DRAFT contracts |
| `editor` | All writer permissions + submit a contract for review (DRAFT → REVIEW) |
| `approver` | All editor permissions + promote contracts to ACTIVE (REVIEW → ACTIVE); bump version |
| `auditor` | Read-only — validate records, view contracts, history, and audit logs; no writes |
| `admin` | All approver permissions + token management, contract reload, node operations |

Key separation of duties:
- A writer **cannot** submit for review or promote a contract to ACTIVE (prevents self-approval).
- **ACTIVE contracts are immutable.** No role — including `admin` or `approver` — may add, update, or delete rules on an ACTIVE contract. Attempts return `409 Conflict`.
- Use separate tokens for automated pipelines (writer role) vs human reviewers (approver role).
...
The old ACTIVE version is automatically set to DEPRECATED at step 1. Both versions remain in the history table with full hash-chained audit trails.
...
Save this as `docker-compose.prod.yml` alongside your main `docker-compose.yml`:
...
Generate the required secrets before first deploy:

```bash
# Generate SECRET_KEY
SECRET_KEY=$(python -c "import secrets; print(secrets.token_hex(32))")

# Generate UI_ACCESS_TOKEN (requires AUTH_MODE=token to already be set)
