...
A standard pattern for agents that need to self-correct:
...
def validate_and_fix(contract: str, record: dict, fix_fn) -> dict:
    """
    Validate a record; if it fails, get remediation hints and call fix_fn,
    then re-validate. Escalate to human if still failing after MAX_RETRIES.

    fix_fn(record, hints) -> dict  — your LLM or rule-based fix function
    """
    for attempt in range(MAX_RETRIES + 1):
        ...
        if attempt == MAX_RETRIES:
            return {
                ...
                "message": f"Validation failed after {MAX_RETRIES + 1} attempts",
            }

        # Get remediation hints for each error
        hints = []
        ...
        # Ask your LLM / fix function to correct the record
        record = fix_fn(record, hints)
...
# Step 2: Create a draft contract via the MCP tool (or REST API equivalent below)
# Via REST API:
draft_resp = requests.post(f"{OPENDQV_URL}/api/v1/contracts", json={
    "name": "MCP_satellite_telemetry",
    "description": "Validates satellite telemetry records",
    "owner": "ACME Corp",
    ...
    "rules": [
        ...
        {"name": "velocity_range", "type": "range", "field": "velocity_ms",
         "min": 7000.0, "max": 8000.0, "error_message": "velocity_ms must be 7000–8000 m/s"},
        {"name": "signal_quality_range", "type": "range", "field": "signal_quality",
         "min": 0.0, "max": 1.0, "error_message": "signal_quality must be 0.0–1.0"},
        {"name": "timestamp_required", "type": "not_empty", "field": "timestamp",
         "error_message": "timestamp is required"},
        {"name": "status_required", "type": "not_empty", "field": "status",
