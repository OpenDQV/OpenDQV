...
When OpenDQV rejects records (`validation.failed`), push a custom event directly to Monte Carlo to flag the affected asset.
...
| **Q2 2026** | Add `validation.failed` webhook → MC custom event receiver |
| **Q3 2026** | Build quality-trend dashboard in MC using OpenDQV pass-rate data |
| **Q4 2026** | Native MC connector (planned) |
