...
| `status` | `DatasetProperties.customProperties["contract_status"]`; `archived` contracts are skipped |
...
## Approach 3 — Webhook-Driven `opendqv.validation.failed` → Real-Time Tagging
...
OpenDQV engine
     │
     │  POST /webhooks/datahub (opendqv.validation.failed event)
     ▼
...
| **Planned — based on community demand** | Add Approach 5 (hash-based incremental sync) to reduce API call volume |
| **Planned — based on community demand** | Deploy Approach 3 (webhook receiver) for real-time failure tagging |
| **Planned — based on community demand** | Add Approach 2 (quality trend score) to surface pass rates in DataHub dashboards |
| **Planned — based on community demand** | Evaluate Approach 4 (Assertions) if your team uses DataHub's assertion monitor UI |
