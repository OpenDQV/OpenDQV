...
def validate_micro_batch(batch_df, batch_id):
    ...
    result = client.validate_batch(
        records,
        contract="orders",
        trace_id=f"autoloader:batch:{batch_id}",
    )
...
run_id = dbutils.widgets.get("run_id")  # passed by Databricks job
result = client.validate_batch(records, contract="orders", trace_id=run_id)
...
| **Q3 2026** | Switch Kafka → Delta pipelines to `foreachBatch` streaming validation |
| **Q3 2026** | Add OpenDQV validation task to Databricks Asset Bundle workflow definitions |
| **Q4 2026** | Databricks Partner Connect integration (planned) |
