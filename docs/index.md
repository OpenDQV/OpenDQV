...
Short visual walkthroughs of OpenDQV in action across the integrations it supports. Every demo here is a real, runnable scenario from the repository.

| Demo | What it shows |
|---|---|
| [![MCP demo](demo_mcp_poster.png)](demo_mcp.mp4)<br>**[Claude Desktop + OpenDQV MCP + Marmot](mcp.md)** | Claude Desktop calls the OpenDQV MCP server to validate a menu item against `ppds_menu_item` (Natasha's Law allergen compliance), then composes with the Marmot catalog MCP server to surface ownership and lineage. **[▶ Play the 4-minute demo](demo_mcp.mp4)** |
...
| ![Pandas demo](demo_pandas.gif)<br>**[Pandas integration](pandas_integration.md)** | DataFrame validation via `LocalValidator` before `to_sql()`. |
