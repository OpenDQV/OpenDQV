...
| A data store | No — OpenDQV is stateless; it validates records, not tables |
...
Use OpenDQV webhooks to trigger a GX Checkpoint run when `validation.failed` fires — letting GX scan the affected dataset immediately after a write-time rejection cluster is detected.
...
| **Q2 2026** | Integrate incremental hash-based sync into your CI/CD pipeline |
| **Q3 2026** | Add webhook-triggered Checkpoint runs for near-real-time correlation |
| **Q4 2026** | Native GX Data Context plugin for push-based contract sync |
