...
> All items are planned based on community demand and feedback. None are committed delivery dates.
...
| Monte Carlo | `opendqv.validation.failed` webhook → MC custom event receiver |
| DataHub | Webhook receiver — push `opendqv.validation.failed` events to DataHub as assertions |
...
| Area | Item |
|------|------|
| dbt | `opendqv_dbt` macro package for singular tests |
...
| Collibra | Webhook → Collibra workflow trigger for `opendqv.validation.failed` events |
