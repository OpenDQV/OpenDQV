...
Contracts move through three states: `draft` → `active` → `deprecated`.

- `draft`: validation requests against this contract return a 422 (blocked by lifecycle guard).
- `active`: normal operation.
- `deprecated`: contract is excluded from listings but remains in history.
...
2. Set the contract to `deprecated` to remove it from active validation:
