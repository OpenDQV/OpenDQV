...
def load_records(records: list[dict], contract: str, table: str):
    ...
    if rejected:
        cur.executemany(
            ...
            [(table, str(r["record"]), str(r["errors"])) for r in rejected],
        )
        conn.commit()
...
| **Q2 2026** | Create an External Function for SQL-level validation of landing tables |
| **Q3 2026** | Add Streams + Tasks for serverless event-driven validation within Snowflake |
| **Q4 2026** | Snowflake Native App packaging (planned) |
