...
## 4. TRACE_LOG WORM Storage Setup
...
Every build is scanned by **Trivy** in CI (`container-scan` job in `.github/workflows/ci.yml`). Results are uploaded to the GitHub Security tab as SARIF. CRITICAL findings with a fix block the build; HIGH findings without an upstream fix are surfaced as warnings only (`--ignore-unfixed`).
...
- The `container-scan` CI job runs on every push to `main` and `develop` and on all PRs to `main`.
