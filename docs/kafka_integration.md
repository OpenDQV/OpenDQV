...
try:
    while True:
        ...
        result = client.validate(record, contract="orders", trace_id=trace_id)
        ...
        if result["valid"]:
            ...
            producer.produce(
                ...
                key=msg.key(),
                value=msg.value(),
                headers={"x-trace-id": trace_id, "x-contract": "orders"},
            )
            producer.poll(0)
...
async def validate_and_route(client, producer, msg):
    ...
    async with semaphore:
        record = json.loads(msg.value)
        trace_id = f"kafka:{msg.topic}:{msg.partition}:{msg.offset}"
        result = await client.validate(record, contract="orders", trace_id=trace_id)
...
try:
    while True:
        ...
        result = client.validate_batch(records, contract="orders", trace_id=trace_id)
...
## Approach 5 — Webhook Back-Channel: `validation.failed` → Kafka Alert Topic

Publish a Kafka alert message when OpenDQV fires a `validation.failed` webhook. Lets your existing Kafka-native alerting consumers pick up OpenDQV rejection events without polling.
...
async def handle_validation_failed(request: Request):
    ...
    alert_producer.produce(
        ...
        headers=[("x-event", b"validation.failed")],
    )
...
def get_contract_hash(contract: str) -> str:
    ...
    if now - _last_check > HASH_TTL_SECONDS:
        ...
        for c in resp.json():
            _hash_cache[c["name"]] = c["contract_hash"]
        _last_check = now
...
| **Q2 2026** | Switch to async batch validation (Approach 2+3) for throughput above ~500 msg/s |
| **Q3 2026** | Add webhook → Kafka alert topic for rejection cluster monitoring (Approach 5) |
