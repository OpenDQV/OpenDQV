...
_EXCLUDED_TEMPLATES = {
    "sf_contact", "sf_lead", "customer_onboarding", "financial_services_customer",
    "books",             # wizard-generated demo contract
    "imported_contract", # empty import placeholder
    "unnamed_suite",     # 0-rule GX import artifact
}
...
class OnboardingWizard:
    ...
    def _list_templates(self) -> list[dict]:
        ...
        for path in sorted(self.contracts_dir.glob("*.yaml")):
            ...
            try:
                ...
                # contract_name is the YAML 'name:' field — used for API validation calls.
                # 'name' (path.stem) is the filename key used for template lookup.
                # They differ: e.g. filename = social_media_age_compliance,
                # contract name = age_compliance_record.
    ...
    def run(self) -> WizardResult:
        ...
        if use_template:
            # Use contract_name (YAML 'name:' field) for API calls — it may differ
            # from the filename stem (e.g. social_media_age_compliance.yaml has name: age_compliance_record)
            ...
            entity = raw_entity.lower().strip().replace(" ", "_").replace("-", "_") or "customer"
            self.result.entity = entity
