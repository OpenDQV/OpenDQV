...
## For CTOs and Procurement Teams
...
**Full source visibility.** MIT licence — every line is publicly readable on GitHub. No proprietary components, no telemetry, no phone-home behaviour.

**Observation-only mode.** Run validation in observation-only mode before any blocking goes live. All violations are logged to the audit trail with `mode=observation_only` — nothing is blocked. Use the observation report to quantify what would have been caught, and to have evidence-based conversations with data suppliers before enforcement starts.
