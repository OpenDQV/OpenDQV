...
if echo "$AUDIT" | grep -q "Chain integrity: PASS"; then
  COUNT=$(echo "$AUDIT" | grep -oP 'All \d+ entries' | grep -oP '\d+' || echo "?")
...
if echo "$HTTP_TEST" | grep -q "passed"; then
  COUNT=$(echo "$HTTP_TEST" | grep -oP '\d+ passed' | grep -oP '\d+')
...
if echo "$CONTRACT_TEST" | grep -q "passed"; then
  COUNT=$(echo "$CONTRACT_TEST" | grep -oP '\d+ passed' | grep -oP '\d+')
...
# =============================================================================
# docker compose exec runs inside the running container → same DB as the service
ADMIN_TOKEN_OUT=$(docker compose exec -T api \
  python -m cli token-generate smoke-admin --role admin --expiry-days 1 2>&1)
ADMIN_PAT=$(echo "$ADMIN_TOKEN_OUT" | grep -oP 'eyJ[A-Za-z0-9._-]+' | head -1)
if [ -z "$ADMIN_PAT" ]; then
  fail "Failed to generate admin PAT via exec"
  echo "    $ADMIN_TOKEN_OUT"
else
  AUTH_HTTP=$(curl -s -o /dev/null -w "%{http_code}" \
    -H "Authorization: Bearer $ADMIN_PAT" \
    http://localhost:8000/api/v1/contracts)
  if [ "$AUTH_HTTP" = "200" ]; then
    ok "Admin PAT → GET /api/v1/contracts returns 200"
  else
    fail "Admin PAT auth failed: HTTP $AUTH_HTTP"
  fi
fi
...
WRITER_TOKEN_OUT=$(docker compose exec -T api \
  python -m cli token-generate smoke-writer --role writer --expiry-days 1 2>&1)
WRITER_PAT=$(echo "$WRITER_TOKEN_OUT" | grep -oP 'eyJ[A-Za-z0-9._-]+' | head -1)
...
# =============================================================================
BATCH_CSV=$(mktemp /tmp/smoke_batch_XXXXXX.csv)
cat > "$BATCH_CSV" << 'EOF'
email,name,age,id
...
if $UI_HEALTHY; then
  ...
  fail "Streamlit UI did not respond with 200 within 30s (last HTTP: $UI_HTTP)"
