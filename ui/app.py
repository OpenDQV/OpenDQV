...
if section == "Contracts":
    ...
    if r and r.status_code == 200:
        ...
        _active_count  = sum(1 for c in _all_contracts if c.get("status") != "deprecated")
        _domain_count  = len(_all_domains)
        _dep_count     = sum(1 for c in _all_contracts if c.get("status") == "deprecated")
        st.caption(
        ...
        contracts = _all_contracts if show_all else [c for c in _all_contracts if c.get("status") != "deprecated"]
        ...
        if contracts:
            ...
            with st.expander("⚙️ Industry setup — focus this workbench on your domain"):
                ...
                with _setup_col1:
                    if _to_deprecate:
                        st.caption(f"{len(_to_deprecate)} contracts will be deprecated.")
                    else:
                    ...
                    if _to_deprecate and st.button("Apply — archive others", type="primary", key="bulk_deprecate"):
                        ...
                        for _bc in _to_deprecate:
                            _br = api_post(f"/api/v1/contracts/{_bc['name']}/status", params={"status": "deprecated"})
            ...
            df["status"] = df["status"].replace({"deprecated": "archived"})
            ...
            if selected:
                ...
                if detail_r and detail_r.status_code == 200:
                    ...
                    _lc_icon  = {"draft": "🔵", "review": "🟡", "active": "🟢", "deprecated": "🔴"}.get(current, "⚪")
                    _lc_steps = ["draft", "review", "active", "archived"]
                    _lc_display = {"draft": "draft", "review": "review", "active": "active", "archived": "deprecated"}
                    _lc_current_display = "archived" if current == "deprecated" else current
                    _lc_flow  = " → ".join(
                        ...
                        with _lc_col2:
                            with st.expander("⚠️ Archive this contract"):
                                ...
                                if _confirm and st.button("Archive", key=f"deprecate_{selected}"):
                                    _lr = api_post(f"/api/v1/contracts/{selected}/status", params={"status": "deprecated"})
                                    if _lr and _lr.status_code == 200:
                                        _ui_audit("status_change", f"contract={selected} new_status=deprecated")
    ...
    if not contract_names:
        ...
        with tcol3:
            token_role = st.selectbox(
                "Role",
                ["writer", "editor", "approver", "auditor"],
                index=0,
                key="guide_token_role",
                help=(
                    "writer — validate records only (source systems, MCP agents)\n"
                    "editor — validate + submit contracts for review\n"
                    "approver — editor + approve/reject review contracts\n"
                    "auditor — read-only access to contracts and audit logs"
                ),
            )
        with tcol4:
...
""")
            ...
            st.info(
                "Use a `writer` role token for agents that only validate data. "
...
if section == "Webhooks":
    ...
    wh_events = st.multiselect(
        "Events",
        ["validation.failed", "validation.warning", "batch.failed"],
...
if section == "CLI Guide":
    ...
    st.code(
        "# With deprecated contracts included\nopendqv list --all",
        language="bash",
    )
    ...
    st.code(
        "# Writer — validation only (source systems, MCP read agents)\n"
        "opendqv token-generate salesforce-prod --role writer\n\n"
        "# Editor — validation + submit contracts for review\n"
        "opendqv token-generate ci-pipeline --role editor\n\n"
        "# Approver — editor + approve/reject review contracts\n"
        "opendqv token-generate lead-architect --role approver\n\n"
        "# Auditor — read-only (contracts, audit logs, stats)\n"
        "opendqv token-generate compliance-team --role auditor",
        language="bash",
    )
    st.caption(
        "In `AUTH_MODE=open`, all tokens are capped to `writer` regardless of `--role`. "
