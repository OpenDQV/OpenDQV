...
def create_pat(username: str, scopes: str = "read:write", expiry_days: int = None, role: str = "writer") -> dict:
    """
...
async def get_current_role(authorization: str = Header(None)) -> str:
    ...
    if not authorization or not authorization.startswith("Bearer "):
        return "writer"  # unauthenticated → least-privileged role

    token = authorization.split("Bearer ", 1)[1].strip()
    try:
        payload = jwt.decode(token, SECRET_KEY, algorithms=[ALGORITHM])
        return payload.get("role", "writer")
    except JWTError:
        return "writer"
